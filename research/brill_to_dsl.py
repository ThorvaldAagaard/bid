...
def render(rules: List[OutRule], stats: Counter, total_rows: int,
    ...
    out.append("#     especially for slams and sacrifices.")
    out.append("#")
    out.append("#   * MEASURED, NOT ASSUMED (§6.56). An earlier header")
    out.append("#     claimed this system \"will underbid rather than")
    out.append("#     overbid, which is the safe direction\". That was an")
    out.append("#     assumption, never a measurement, and it has been")
    out.append("#     retracted in both directions — the system is not")
    out.append("#     reliably better OR worse, it is INCOMPLETE and")
    out.append("#     higher-variance.")
    out.append("#")
    out.append("#     FIRST-CALL-ONLY. Every rule requires")
    out.append("#     my_last_call == 'NONE'. Measured over 200 boards")
    out.append("#     (1,727 real calls) it can make 60.6% of first calls")
    out.append("#     (485/800) and 0% of the 927 later ones — 28.1%")
    out.append("#     overall. Auctions cannot continue past one round,")
    out.append("#     and 120/400 boards (30%) are passed out entirely.")
    out.append("#")
    out.append("#     RANKING DEPENDS ON THE METRIC (400 boards):")
    out.append("#       abs deviation from par : champion better by 0.92")
    out.append("#         IMP/board (t 4.31, 95% CI 0.50-1.34)")
    out.append("#       signed 'beating par'   : DEAD HEAT, -0.32 vs")
    out.append("#         champion's -0.35 (diff -0.03, CI -0.68..+0.62)")
    out.append("#     It beats par on 179 boards vs champion's 173 and by")
    out.append("#     more (+7.70 vs +6.87), but loses on 198 vs 172. A")
    out.append("#     wider spread, not a worse system.")
    out.append("#")
    out.append("#     A hybrid (this file where a rule matches, champion")
    out.append("#     elsewhere) is indistinguishable from champion alone")
    out.append("#     on BOTH metrics: the part that DID convert adds")
    out.append("#     ~0.01 IMP/board.")
    out.append("#")
    out.append("#     Treat this file as a position catalogue, not a system.")
    out.append("#     But note (§6.57): REMOTE Brill, scored the same way,")
    out.append("#     beats champion by +0.98 abs IMP/board (t 3.74) with")
    out.append("#     1 pass-out in 250. The target is worth ~2.0 IMP/board")
    out.append("#     more than this file renders — so the failure is the")
    out.append("#     2-deep capture, not Brill. Distil from /bid traces")
    out.append("#     rather than translating rules.")
